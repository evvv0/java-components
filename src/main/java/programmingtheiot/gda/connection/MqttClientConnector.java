...
public class MqttClientConnector implements IPubSubClient, MqttCallbackExtended
{
   ...
   */
    ...
    public boolean connectClient()
    {
        try {
            ...
            if (!this.mqttClient.isConnected()) {
                ...
                _Logger.info("MQTT client successfully connected to broker: " + this.brokerAddr);
                return true;
            } else {
        ...
        try {
            if (this.mqttClient != null) {
                if (this.mqttClient.isConnected()) {
                    ...
                    _Logger.info("MQTT client successfully disconnected from broker: " + this.brokerAddr);
                    return true;
                } else {
    ...
    public boolean publishMessage(ResourceNameEnum topicName, String msg, int qos)
    {
        ...
        // Validar el QoS, si no es válido, establecer el valor por defecto
        if (qos < 0 || qos > 2) {
            qos = ConfigConst.DEFAULT_QOS;  // Usar valor por defecto
        }

        try {
            // Convertir el mensaje en bytes
            byte[] payload = msg.getBytes();
            // Crear el objeto MqttMessage
    ...
    public boolean subscribeToTopic(ResourceNameEnum topicName, int qos)
    {
        ...
        try {
            // Suscribirse al tema con el QoS proporcionado
            this.mqttClient.subscribe(topicName.getResourceName(), qos);
            _Logger.info("Successfully subscribed from topic: " + topicName.getResourceName());
            return true;
        } catch (Exception e) {
    ...
    {
        // Log cuando un mensaje llega al cliente
        ...
        public void messageArrived(String topic, MqttMessage message) throws Exception {
            try {
                ...
                if (dataMsgListener != null) {
                    dataMsgListener.handleActuatorCommandResponse(resource, actuatorData);
                }
            } catch (Exception e) {
                _Logger.warning("Failed to convert message to ActuatorData.");
            }
        }
    }
    ...
    private class SensorDataMessageListener implements IMqttMessageListener
    {
        ...
        public void messageArrived(String topic, MqttMessage message) throws Exception {
            try {
                ...
                _Logger.info("Received SensorData message: " + sensorData.getName());

                if (dataMsgListener != null) {
                    dataMsgListener.handleSensorMessage(resource, sensorData);
                }
            } catch (Exception e) {
                _Logger.warning("Failed to convert message to SensorData.");
            }
        }
    }
    ...
    private class SystemPerformanceDataMessageListener implements IMqttMessageListener
    {
        ...
        public void messageArrived(String topic, MqttMessage message) throws Exception {
            try {
                ...
                if (dataMsgListener != null) {
                    dataMsgListener.handleSystemPerformanceMessage(resource, sysPerfData);
                }
            } catch (Exception e) {
                _Logger.warning("Failed to convert message to SystemPerformanceData.");
            }
        }
    }}
